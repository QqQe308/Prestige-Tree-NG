addLayer("p", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
			...
			if (hasUpgrade("p", 21)) mult = mult.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e50:9);
        ...
        gainExp() { // Calculate the exponent on main currency from bonuses
			...
			if (hasUpgrade("p", 31)) exp = exp.times(1.15);
			return exp;
        },
		...
		upgrades: {
			...
			11: {
				title: "Begin",
				description: "Generate 1? No, 10 Points every second.",
			...
			12: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?10:5).pow(tmp.h.costExp11) },
				effect() {
					if (inChallenge("ne", 11)) return new Decimal(1);
					
					let eff = player.p.points.plus(2).pow(0.4);
			...
			13: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?50:15).pow(tmp.h.costExp11) },
				effect() { 
					let eff = player.points.plus(1).log(2).pow(0.75).plus(1);
			...
			21: {
				title: "More Prestige",
				description() { return "Prestige Point gain is increased by "+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e52":"800")+"%." },
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e171:25).pow(tmp.h.costExp11) },
			...
			22: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e262:1500).pow(tmp.h.costExp11) },
				effect() {
					let eff = Decimal.pow(2, player.p.upgrades.length);
				...
				formula() { 
					...
					return exp.gt(1)?("(2^x)^"+format(exp)):"2^x" 
				},
			},
			23: {
				title: "Reverse Prestige Boost",
				description: "Prestige Point gain is boosted by your Points.",
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e305:2e4).pow(tmp.h.costExp11) },
				effect() {
					let eff = player.points.plus(1).log(20).sqrt().plus(1);
					if (hasUpgrade("p", 33)) eff = eff.pow(upgradeEffect("p", 33));
			...
			31: {
				title: "Oh…That's weird",
				description: "Prestige Point gain is raised to the power of 1.15.",
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e316":5e5).pow(tmp.h.costExp11) },
				unlocked() { return hasAchievement("a",23) },
			},
			32: {
				title: "Still Useless",
				description: "<b>Upgrade Power</b> is squared.",
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e355":1e9).pow(tmp.h.costExp11) },
			...
			33: {
				...
				cost() { return tmp.h.costMult11.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e436":1e12).pow(tmp.h.costExp11) },
				effect() { return player.p.total.plus(1).log(5).plus(1).log(5).div(3).plus(1).times(hasUpgrade("hn", 33) ? upgradeEffect("hn", 33) : 1) },
...
addLayer("b", {
        ...
        color: "#6e64c4",
        requires() { return new Decimal(20000).times((player.b.unlockOrder&&!player.b.unlocked)?5000:1) }, // Can be a function that takes requirement increases into account
		...
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.5:500 },
		gainMult() { 
		...
		addToBase() {
			...
			if (hasUpgrade("b", 22)) base = base.plus(8);
		...
		milestones: {
			...
			1: {
				...
				done() { return player.b.best.gte(6) || hasAchievement("a", 71) },
		...
		upgrades: {
			...
			12: {
				...
				cost() { return tmp.h.costMult11b.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1250:3) },
				effect() {
					let ret = player.g.points.add(1).log(2).sqrt().div(2).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("b", 14));
					return ret;
				},
			...
			13: {
				...
				effect() { 
					let ret = player.p.total.add(1).log(2).add(1).log(2).div(3).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1) 
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("b", 14));
					return ret;
				},
			...
			21: {
				title: "Gen Z^10??",
				description: "The Generator Power effect is ^10.",
				cost() { return tmp.h.costMult11b.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2000:5) },
				unlocked() { return hasUpgrade("b", 11) && hasUpgrade("b", 12) },
			},
			22: {
				title: "Up to the 100th Floor",
				description: "The Booster Effect base +8.",
				cost() { return tmp.h.costMult11b.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2075:14) },
			...
			32: {
				...
				cost() { return tmp.h.costMult11b.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1438:78) },
				unlocked() { return hasAchievement("a", 41) },
			},
			33: {
				title: "Even More Additions",
				description: "<b>More Additions</b> is stronger based on your Super Boosters.",
				cost() { return tmp.h.costMult11b.times(84) },
				unlocked() { return hasAchievement("a", 41) },
				effect() { return player.sb.points.times(player.sb.points.gte(4)?15:10).plus(1).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?3:1) },
...
addLayer("g", {
        ...
        color: "#a3d9a5",
        requires() { return new Decimal(20000).times((player.g.unlockOrder&&!player.g.unlocked)?5000:1) }, // Can be a function that takes requirement increases into account
		...
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2.5:500 },
		gainMult() {
			let mult = new Decimal(1);
		...
		powerExp() {
			...
			if (hasUpgrade("b", 21)) exp = exp.times(10);
		...
		milestones: {
			...
			1: {
				...
				done() { return player.g.best.gte(5) || hasAchievement("a", 71) },
				effectDescription: "You gain 100% of Prestige Point gain every second.",
			},
			2: {
				requirementDescription: "8 Generators",
				done() { return player.g.best.gte(8) || hasAchievement("a", 71) },
		...
		upgrades: {
			...
			12: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?375:3) },
				effect() { 
					let ret = player.b.points.add(1).log(2).sqrt().div(2).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
			...
			14: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?378:11) },
...
addLayer("t", {
        ...
        color: "#006609",
        requires() { return new Decimal("1e520").times(Decimal.pow("1e180", Decimal.pow(player[this.layer].unlockOrder, 1.415038))) }, // Can be a function that takes requirement increases into account
...
addLayer("e", {
        ...
        color: "#b82fbd",
        requires() { return new Decimal('1e400').times(Decimal.pow("1e180", Decimal.pow(player[this.layer].unlockOrder, 1.415038))) }, // Can be a function that takes requirement increases into account
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
			...
			if (hasUpgrade("sb", 12)) mult = mult.times(upgradeEffect("sb", 12));
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            exp = new Decimal(1)
            if(hasAchievement("a",43)) exp = exp.add(0.15)
            return exp
        },
		...
		freeEnh() {
			...
			if (hasUpgrade("e", 21)) enh = enh.plus(5);
		...
		upgrades: {
			...
			11: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e98000":((player.e.unlockOrder>=2)?25:1000)) },
			...
			12: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e98000":(player.e.unlockOrder>=2?400:2e4)) },
			...
			13: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e1e5":1e5) },
				unlocked() { return hasUpgrade("e", 11) },
			},
			14: {
			...
			21: {
				title: "Enhance Plus Plus Plus…",
				description: "Get another FIVE free Enhancers",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e1.01e5":1e7) },
				unlocked() { return hasUpgrade("e", 13) },
			},
			22: {
				title: "Enhanced Reversion",
				description: "Time & Space act as you chse them second.",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e1.01e5":1e9) },
				unlocked() { return (hasUpgrade("e", 21))&&hasUpgrade("e", 12) },
				onPurchase() { player.t.unlockOrder = 1; 
				  player.s.unlockOrder = 1
				},
			},
			23: {
				title: "Enter the E-Space",
				description: "Space Energy provides free Enhancers.",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e1.01e5":1e58) },
				unlocked() { return hasAchievement("a", 33) },
				effect() {
			...
			24: {
				...
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e1.011e5":6.2e62) },
				unlocked() { return hasAchievement("a", 33) },
				effect() { return Decimal.pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e2000":1.25, player.b.points.plus(player.g.points)) },
				effectDisplay() { return format(tmp.e.upgrades[24].effect)+"x" },
				formula() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e2,000":"1.25")+"^(boosters+generators)" },
			},
			31: {
				title: "Amplification",
		...
		milestones: {
			0: {
				...
				done() { return player.e.best.gte(10) || hasAchievement("a", 71) },
				effectDescription: "Keep Booster/Generator milestones on reset.",
			},
			1: {
				requirementDescription: "300 Enhance Points",
				done() { return player.e.best.gte(300) || hasAchievement("a", 41) || hasAchievement("a", 71) },
				effectDescription: "Keep Prestige Upgrades on reset.",
			},
			2: {
				requirementDescription: "2000 Enhance Points",
				done() { return player.e.best.gte(2000) || hasAchievement("a", 71) },
...
addLayer("sb", {
		...
		gainMult() { 
			let mult = new Decimal(1);
			if (hasUpgrade("sb", 11)) mult = mult.div(1.2);
			if (hasUpgrade("ss", 21)) mult = mult.div(1.25);
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) mult = mult.div(4/3);
			return mult;
		},
		doReset(resettingLayer) { 
			let keep = []
			if (hasMilestone("q", 5) || hasAchievement("a", 64)) keep.push("upgrades")
			if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
        },
		...
		effectBase() {
			...
			if (hasChallenge("h", 12)) base = base.plus(2.5);
		...
		upgrades: {
			...
			11: {
				title: "Superior",
				description: "Super-Boosters are 20% cheaper.(at least 72)",
				unlocked() {return player.sb.best.gte(3)},
				cost() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1:4 },
			},
			12: {
				title: "Enhancing-Boosters",
				description: "Boost Enhance points based on your super-booster points.",
				cost() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1:6 },
				unlocked() { return player.sb.best.gte(5) },
				effect() { 
					return player.sb.points.add(1).pow(player.sb.points).pow(2)
				},
				effectDisplay() { return format(tmp.sb.upgrades[12].effect)+"x" },
				formula: "10^x",
			},
			13: {
				title: "Generating-Boosters",
				description: "Increase Super-Generators' effect base based on your boosters",
				cost() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1:14 },
				unlocked() { return player.sb.best.gte(13) },
				effect() { 
					return player.b.points.add(1).log(2).add(1)},
				effectDisplay() { return "+" + format(tmp.sb.upgrades[13].effect)},
				formula: "log2(x+1)",
			},
		},
		tabFormat: ["main-display",
...
addLayer("sg", {
        ...
        color: "#248239",
        requires: new Decimal(175), // Can be a function that takes requirement increases into account
		...
		gainMult() { 
			let mult = new Decimal(1);
			if (hasUpgrade("ss", 21)) mult = mult.div(1.25);
			if (hasUpgrade("sg", 11)) mult = mult.div(1.2);
		...
		effectBase() {
			...
			if (hasUpgrade("sb", 13)) base = base.plus(upgradeEffect("sb", 13));
		...
		doReset(resettingLayer){ 
			let keep = []
			if (hasMilestone("o", 0) || hasAchievement("a", 64)) keep.push("upgrades")
            if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
        },
		spectralTotal() {
		...
		upgrades: {
			...
			cols: 4,
			11: {
				title: "General-Generate",
				description: "Super-Generators are 20% cheaper.(at least 175)",
				unlocked() {return player.sg.best.gte(9)},
				cost() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1:11 },
			},
		},
		tabFormat: ["main-display",
			"prestige-button",
...
addLayer("h", {
		...
		challenges: {
			...
			11: {
				...
				goal() { return new Decimal(player.ma.current=="h"?"e1.37e8":"1e4500") },
			...
			12: {
				...
				goal() { return new Decimal(player.ma.current=="h"?"e5e8":"1e640") },
				currencyDisplayName: "points",
				currencyInternalName: "points",
				rewardDescription() { return "Add 2.5 to the Super Booster base"+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?(" and multiply it by your Hyperspace Energy"):"")+"." },
			},
			21: {
				name: "Out of Room",
				completionLimit: 1,
				...
				goal() { return new Decimal(player.ma.current=="h"?"e5.7e7":"1e625") },
				currencyDisplayName: "generator power",
				currencyInternalName: "power",
				currencyLayer: "g",
			...
			31: {
				...
				goal() { 
					...
					return Decimal.pow("1e500", Decimal.pow(comps, 2.5)).times("1e11000") 
				},
				completeInBulk() {
					if (challengeCompletions("h", 31)>=tmp[this.layer].challenges[this.id].completionLimit) return;
					let target = player.points.div("1e11000").max(1).log("1e500").root(2.5)
...
addLayer("q", {
        ...
        color: "#c20282",
        requires: new Decimal("1e540"), // Can be a function that takes requirement increases into account
		...
		milestones: {
			...
			4: {
				...
				done() { return player.q.total.gte(5) || hasAchievement("a", 71) },
			...
			5: {
				...
				effectDescription: "Time, Space, & Super-Boosters reset nothing, keep Super-booster upgrades on reset, and you can destroy individual Space Buildings.",
			},
			6: {
				unlocked() { return hasChallenge('h',21)},
			...
			7: {
				unlocked() { return hasChallenge('h',22)},
				requirementDescription: "1e25 Total Quirks",
				done() { return player.q.total.gte(1e25) || hasAchievement("a", 71) },
		...
		upgrades: {
			...
			22: {
				...
				cost() { return player.q.time.plus(1).pow(4.2).times(1e11).pow(player.ma.current=="q"?(this.id*2):1) },
				costFormula: "1e11*(time+1)^4.2",
				currencyDisplayName: "quirk energy",
				currencyInternalName: "energy",
				currencyLayer: "q",
			...
			41: {
				...
				cost() { return new Decimal((player.ma.current=="q")?"1e2325":1e160) },
				currencyDisplayName: "quirk energy",
				currencyInternalName: "energy",
				currencyLayer: "q",
			...
			42: {
				...
				cost() { return new Decimal((player.ma.current=="q")?"1e3675":1e180) },
				currencyDisplayName: "quirk energy",
				currencyInternalName: "energy",
				currencyLayer: "q",
				unlocked() { return hasUpgrade("q", 41) },
			...
			43: {
				...
				cost() { return new Decimal((player.ma.current=="q")?"1e5340":1e215) },
				currencyDisplayName: "quirk energy",
				currencyInternalName: "energy",
				currencyLayer: "q",
			...
			44: {
				...
				cost() { return new Decimal((player.ma.current=="q")?"1e8725":1e308) },
				currencyDisplayName: "quirk energy",
				currencyInternalName: "energy",
				currencyLayer: "q",
...
addLayer("o", {
	name: "solarity", // This is optional, only used in a few places, If absent it just uses the layer id.
        ...
        requires() { 
			let req = new Decimal((player[this.layer].unlockOrder>0&&!hasAchievement("a", 62))?17:15).sub(tmp.o.solEnEff);
		...
		milestones: {
			0: {
				requirementDescription: "20,000 Total Solarity",
				done() { return player.o.total.gte(2e4) || hasAchievement("a", 71) },
				effectDescription: "Gain 50% of Solarity gain every second. Keep Super-Generator upgrades on Reset",
			},
		},
})//o
...
addLayer("ss", {
        ...
        color: "#e8ffff",
        requires() { return new Decimal((player[this.layer].unlockOrder>0&&!hasAchievement("a", 62))?38:36) }, // Can be a function that takes requirement increases into account
        requires() { return new Decimal((player[this.layer].unlockOrder>0&&!hasAchievement("a", 62))?38:36) }, // Can be a function that takes requirement increases into account
		roundUpCost: true,
		...
		effect() { 
			if (!unl(this.layer)) return new Decimal(1);
			let gain = Decimal.pow(tmp.ss.effBase, player.ss.points).sub(1).mul(3);
		...
		upgrades: {
			...
			21: {
				...
				description: "Super Boosters & Super Generators are 25% cheaper.",
			...
			31: {
				...
				cost() { return new Decimal((player.ma.current=="ss")?1626:54) },
			...
			32: {
				...
				cost() { return new Decimal((player.ma.current=="ss")?1628:56) },
				currencyDisplayName: "space energy",
				currencyInternalName: "points",
				currencyLayer: "s",
				unlocked() { return hasUpgrade("ss", 31) },
				effect() { return player.q.buyables[11].sqrt()},
			...
			33: {
				...
				cost() { return new Decimal((player.ma.current=="ss")?"1e17796":2.5e8) },
				currencyDisplayName: "subspace",
				currencyInternalName: "subspace",
				currencyLayer: "ss",
			...
			41: {
				...
				cost() { return new Decimal((player.ma.current=="ss")?1628:58) },
				currencyDisplayName: "space energy",
				currencyInternalName: "points",
				currencyLayer: "s",
...
addLayer("a", {
        ...
        achievements: {
			...
			22: {
				...
				tooltip: "Unlock Boosters & Generators. Reward:Points x10.",
				image: "images/achs/22.png",
			},
			23: {
				name: "Prestige^2.3",
				done() { return player.p.points.gte(1e5) },
				tooltip: "Reach 1e5 Prestige Points. Reward: Unlock 3 new Prestige Upgrades.",
			...
			32: {
				name: "Why no meta-meta-layer?",
				done() { return player.points.gte("3.23e616") },
				tooltip: "Reach 3.23e616 Points. Reward: Double Prestige Point gain.",
			...
			43: {
				...
				done() { return player.e.points.gte(1e80) },
				tooltip: "Reach 1e80 Enhance Points. Reward: Enhance points ^1.15 .",
				image: "images/achs/43.png",
			},
			44: {
				name: "Space is for Dweebs",
				done() { return tmp.s.manualBuildingLevels.eq(0) && player.g.power.gte("1e233") },
				tooltip: "Reach 1e233 Generator Power without any Space Buildings. Reward: Points ^1.1 .",
			...
			52: {
				...
				done() { return inChallenge("h", 11) && player.points.gte("1e7500") },
				tooltip: 'Reach 1e7,500 Points in "Upgrade Desert".',
			...
			54: {
				...
				done() { return player.sg.best.eq(0) && player.sb.best.eq(0) && player.points.gte("1e27000") },
				tooltip: "Reach 1e27,000 Points without Super-Boosters & Super-Generators.",
			...
			64: {
				name: "Timeless^1.7",
				done() { return player.h.challenges[31]>=7 },
				tooltip: 'Complete "Timeless" 7 times. Reward: Always keep Row 2 & 3 Upgrades.',
